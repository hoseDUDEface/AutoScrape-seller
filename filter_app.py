...
class MTGFilterApp:
    def __init__(self, root):
        ...
        # Results box
        self.results_box = scrolledtext.ScrolledText(root, width=150, height=20)
        self.results_box.grid(row=len(filters)+3, column=0, columnspan=2, pady=5)
    ...
    def display_results(self, df):
        ...
        if df.empty:
            self.results_box.insert(tk.END, "No cards matched the filter.\n")
            return

        for _, row in df.iterrows():
            text = f"Card: {row['name']:30} | Rarity: {row['rarity']:6} | Price: {row['price']:.2f} | Mana Cost: {row['manaCost']} | Type: {row['type']} | Stats: {row['power']}/{row['toughness']}\n"
            text += f"Text: {row['text']}\n\n"
    ...
    def export_filtered(self):
        ...
        if file_path:
            try:
                self.last_filtered_df.to_csv(file_path, index=True)
                messagebox.showinfo("Export Successful", f"Exported {len(self.last_filtered_df)} cards.")
            except Exception as e:
