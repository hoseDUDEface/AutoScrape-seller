...
class MTGFilterApp:
    def __init__(self, root):
        ...
        self.card_tooltips = {}
        self.card_images = {}  # {card_name: PhotoImage}
    ...
    def fetch_scryfall_data(self, card_names, is_foil=True):
        identifiers = []
        card_info = {}
        ...
        for card in response.json()['data']:
            name = card['name']
            price = card['prices']['eur_foil' if is_foil else 'eur'] or "?"
            images = []

            if 'image_uris' in card:
                images.append(card['image_uris']['border_crop'])
            elif 'card_faces' in card:
                for face in card['card_faces']:
                    if 'image_uris' in face:
                        images.append(face['image_uris']['border_crop'])

            card_info[name] = {'price': price, 'image_urls': images}

        return card_info

    def show_tooltip(self, widget, images):
        tooltip = tk.Toplevel(widget)
        tooltip.wm_overrideredirect(True)
        tooltip.wm_geometry("+%d+%d" % (widget.winfo_rootx() + 40, widget.winfo_rooty() + 10))
        frame = tk.Frame(tooltip, bg="white", borderwidth=2, relief="solid")
        frame.pack()
        ...
        self.card_tooltips[widget] = tooltip

    def hide_tooltip(self, widget):
        if widget in self.card_tooltips:
            self.card_tooltips[widget].destroy()
            del self.card_tooltips[widget]
    ...
    def display_results(self, df):
        ...
        for _, row in df.iterrows():
            ...
            # Get image and price
            name = row['name']
            card_data = card_info.get(name, {})
            price = card_data.get('price', '?')

            image_urls = card_data.get('image_urls', [])
            if image_urls and name not in self.card_images:
                try:
                    pil_images = []
                    for url in image_urls:
                        response = requests.get(url)
                        pil = Image.open(BytesIO(response.content)).resize((240, 340))  # .resize((150, 210))
                        pil_images.append(ImageTk.PhotoImage(pil))

                    self.card_images[name] = pil_images
                except Exception as e:
                    print(f"Failed to load images for {name}: {e}")
                    self.card_images[name] = []

            text = (
                f"{name} | Rarity: {row['rarity']} | Seller: {row['price']}€ | Market: {price}€ | "
                f"Mana: {row['manaCost']} | Type: {row['type']} | Stats: {row['power']}/{row['toughness']}\n"
                f"{row['text']}"
            )

            label = tk.Label(frame, text=text, justify="left", anchor="w", wraplength=1000)
            label.pack(side="left", fill="both", expand=True)

            # Bind tooltip for image
            if name in self.card_images and self.card_images[name]:
                imgs = self.card_images[name]
                label.bind("<Enter>", lambda e, w=label, i=imgs: self.show_tooltip(w, i))
                label.bind("<Leave>", lambda e, w=label: self.hide_tooltip(w))
